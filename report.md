...
In this subsection, the robustness and convergence of the program are tested for actual usage. In this test, input of $N$ and $N_j$ will be set as the same value, while $N_j \times dx = 1$. Note that the actual amount of price grids would be $2 N_j +1$. For instance, please refer to the appendix.

As the result, the program is able to process large grid numbers in reasonable amounts of time. It is observed that when $N$ and $N_j$ goes larger with the aforementioned setting, the result converges approximately to -0.091% of error compared to the analytical value. Despite immaterial error, such calculation error might be improved with setting of $N_j \times dx = 3$ in practice, exact value has to be analyzed case by case. Additionally, the computational time could go up exponentially with more grids set. While the numerical accuracy did not improve significantly after $N = 1000$ in the setting, users shall consider the computational power before increasing either $N$ or $N_j$. Please be aware that $dx$ in the program aims to approximate the differential in continuous case, user shall look for the optimal value of this for better numerical approximation. When $dx$ is greater than optimum, error from discretization would be significant; else when $dx$ is smaller than optimum, error from truncation would be significant. 
