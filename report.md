...
In this study, only a few varieties of smart contracts are included in the discussion. In the market, there are more kinds of crypto assets that are not suitable to apply BSM model. For example, smart contracts rely on multiple assets, economically do not act similar to financial derivatives, or perpetual in nature. To measure their fair value when market information is not available, a generalized model or more modified valuation models could be the approaches for such problems.

The empirical analysis of applying the model was not elaborated. In Section 5.1, neither simulation nor empirical data are supporting the statements and causal induction. Except for data analysis, future research could also focus on disclosure incentives, earnings management, and regulation studies. For instance, how willing an entity to disclose its investment portfolio in crypto assets when it is not required. Especially net asset value may not certainly be higher than implying current practice and valuation method. 
