...
Some factors may be considered significant is not modelled and elaborated. In this subsection, these factors would be explained and providing a rough idea on what can future researchers focus on.

#### 5.2.1 Gas Fee
This model did not consider gas fee while it might seem inappropriate as this cost appears obvious to affect investment and valuation decisions. There are two main reasons to not include them in this study.

Before explaining the first reason, it is essential to acknowledged on how gas fee is computed and decided. In short, gas fee fluctuates positively correlated to the frequency of total transaction activity at the moment. While an empirical study on gas fee is not the focus, one should aware that change in gas fee follows a stochastic process despite deterministic algorithms. In practice, change of gas fee is partially affected by on-chain members' transaction activities, while these activities could be dominating the gas fee sometimes when demands for transaction rushes due to events occurred off-chain. It seemed to be a solution to model gas fee independently as transaction friction to affect the boundary conditions, this is not beneficial, in fact counter-effective, for auditability. As mentioned, gas fee follows a stochastic process which requires random variables to model. However, those random variables are very difficult to be audited in practice and [6] would not be acceptable as unable to analyze their sensitivities similar to deterministic variables. Some may argue that gas fee could be predicted with standard regression analysis and treated normally, this is feasible but yet redundant regarding to the second reason.  
