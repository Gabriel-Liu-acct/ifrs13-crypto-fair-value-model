...
#### 5.2.2 Other Factors
Although the volatility of cryptocurrencies is modeled, its stochastic nature is not included. In this study, as the model is designed for accounting purposes, volatility is assumed constant throughout the contract's life. This assumption is unrealistic in general, as cryptocurrencies could have large fluctuations in their volatility in practice, which could also affect the fair value of any smart contracts. In this study, or even in general accounting practice, disclosing sensitivity analysis may fulfill entities' disclosure responsibilities. However, a better model that includes the change of volatility would be more suitable for decision-making purposes.

In this study, only a limited variety of smart contracts are included in the discussion. In the market, there are more kinds of crypto assets that are not suitable for applying the BSM model. For example, smart contracts that rely on multiple assets, that do not act economically similar to financial derivatives, or that are perpetual in nature. To measure their fair value when market information is not available, a generalized model or more modified valuation models could be approaches for such problems.

The empirical analysis of applying the model was not elaborated upon. In Section 5.1, neither simulation nor empirical data support the statements and causal inference. Beyond data analysis, future research could also focus on disclosure incentives, earnings management, and regulation studies. For instance, how willing an entity is to disclose its investment portfolio in crypto assets when it is not required. In particular, net asset value may not necessarily be higher than implying current practice and valuation method.
